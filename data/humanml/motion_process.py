...
import numpy as np
import os
from data.humanml.common.skeleton import Skeleton
from data.humanml.common.quaternion import *
...
def main():
    ...
    joints_num = 22
    # ds_num = 8
    data_dir = Path("/Users/orrav/Documents/Data/human-feedback/pose_data_raw/joints")
    save_dir1 = data_dir.parent / "new_joints"
    save_dir2 = data_dir.parent / "new_joint_vecs"
    save_dir1.mkdir(exist_ok=True, parents=True)
    ...
    source_list = list(data_dir.glob("*.npy"))
    frame_num = 0
    for source_file in tqdm(source_list):
        source_data = np.load(source_file)[:, :joints_num]
        try:
            dataset, ground_positions, positions, l_velocity = process_file(
                source_data, 0.002
            )
            rec_ric_data = recover_from_ric(
                torch.from_numpy(dataset).unsqueeze(0).float(), joints_num
            )
            np.save(save_dir1 / source_file.name, rec_ric_data.squeeze().numpy())
            np.save(save_dir2 / source_file.name, dataset)
