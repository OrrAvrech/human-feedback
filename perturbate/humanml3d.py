...
from data.humanml.motion_process import (
    recover_from_ric,
    extract_features,
    FID_R,
    FID_L,
...
def save_random_perturbations(
    motion_file_list: list[Path],
    num_joints: int,
    parents: np.array,
    num_iter: int,
    ...
    for i, npy_path in enumerate(motion_file_list):
        try:
            ...
            for i in range(num_iter):
                posed_joint_positions, _ = random_motion_warping(
                    joints, parents, pert_perc=pert_perc
                )
                posed_joints_np = posed_joint_positions.numpy()
                posed_data = extract_features(
                    posed_joints_np,
                    feet_thre=0.002,
...
def main(
    ...
    num_joints: Optional[int] = 22,
    num_iter: Optional[int] = 3,
    ...
    sub_lists = [
        motion_file_list[i : i + motions_per_worker]
